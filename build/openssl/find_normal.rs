...
pub fn get_openssl(target: &str) -> FoundUsing {
    if let Some(include_dir) = try_pkg_config() {
        return FoundUsing::Package { include_dir };
    };

    if let Some(include_dir) = try_vcpkg() {
        return FoundUsing::Package { include_dir };
    };
    ...
    match (lib_dir, include_dir) {
        (Some(lib_dir), Some(include_dir)) => FoundUsing::Paths {
            lib_dir,
            include_dir,
        },
        (lib_dir, include_dir) => {
...
fn try_pkg_config() -> Option<PathBuf> {
    ...
    if lib.include_paths.len() > 1 {
        panic!("more than one openssl include path")
    }

    Some(lib.include_paths[0].clone())
}
