...
#include <stdio.h>
static void
dbgDumpMemBlock(const char* label, const BYTE* buf, int size)
{
    const int Cols = 32;
    int i, j, n = Cols;
    printf("%s ", label);
...
#define  CC_Vendor_QC_Certify             CC_YES
...
#endif
#ifndef CC_Vendor_QC_Certify
#   define CC_Vendor_QC_Certify NO
#endif
#if CC_Vendor_QC_Certify == YES
#define  TPM_CC_Vendor_QC_Certify             (TPM_CC)(0x20000199)
#endif
...
// Definitions specific to a platform, but used by the
// reference code.
...
// COMPUTE uint64 Fingerprint of Implementation.h choices

// bits 0-2 = Architecture fingerprint
// FINGERPRINT_ARCH_VALUE 4-7 reserved for future expansion
#if defined(_X86_)
#   define FINGERPRINT_ARCH 0x0ull
#elif defined(_ARM_)
#   define FINGERPRINT_ARCH 0x1ull
#elif defined(_AMD64_)
#   define FINGERPRINT_ARCH 0x2ull
#elif defined(_ARM64_)
#   define FINGERPRINT_ARCH 0x3ull
#else
#   error "Unexpected architecture"
#endif

#define FINGERPRINT_ARCH_MASK   (0x7ull)

// bits 3-7 = Versioning fingerprint
// bit 3 = version 138
// bits 4-7 = Reserved for future versions
#define FINGERPRINT_SPEC_VERSION (0x1ull << 3)

// bits 8-43 = Algorithm fingerprint
#define FINGERPRINT_ALGOS ( \
    (ALG_RSA               * (0x1ull << 8)) | \
    (ALG_SHA1              * (0x1ull << 9)) | \
    (ALG_HMAC              * (0x1ull << 10)) | \
    (ALG_TDES              * (0x1ull << 11)) | \
    ...
    (ALG_MGF1              * (0x1ull << 13)) | \
    (ALG_XOR               * (0x1ull << 14)) | \
    (ALG_KEYEDHASH         * (0x1ull << 15)) | \
    (ALG_SHA256            * (0x1ull << 16)) | \
    (ALG_SHA384            * (0x1ull << 17)) | \
    (ALG_SHA512            * (0x1ull << 18)) | \
    (ALG_SM3_256           * (0x1ull << 19)) | \
    (ALG_SM4               * (0x1ull << 20)) | \
    (ALG_RSASSA            * (0x1ull << 21)) | \
    (ALG_RSAES             * (0x1ull << 22)) | \
    (ALG_RSAPSS            * (0x1ull << 23)) | \
    (ALG_OAEP              * (0x1ull << 24)) | \
    (ALG_ECC               * (0x1ull << 25)) | \
    (ALG_ECDH              * (0x1ull << 26)) | \
    (ALG_ECDSA             * (0x1ull << 27)) | \
    (ALG_ECDAA             * (0x1ull << 28)) | \
    (ALG_SM2               * (0x1ull << 29)) | \
    (ALG_ECSCHNORR         * (0x1ull << 30)) | \
    (ALG_ECMQV             * (0x1ull << 31)) | \
    (ALG_SYMCIPHER         * (0x1ull << 32)) | \
    (ALG_KDF1_SP800_56A    * (0x1ull << 33)) | \
    (ALG_KDF2              * (0x1ull << 34)) | \
    (ALG_KDF1_SP800_108    * (0x1ull << 35)) | \
    (ALG_CTR               * (0x1ull << 36)) | \
    (ALG_OFB               * (0x1ull << 37)) | \
    (ALG_CBC               * (0x1ull << 38)) | \
    (ALG_CFB               * (0x1ull << 39)) | \
...
// bits 44-47 = RSA key size fingerprint
#if MAX_RSA_KEY_BITS == 2048
#   define FINGERPRINT_RSA_KEYSIZE (ALG_RSA * (0x1ull << 44))
#elif MAX_RSA_KEY_BITS == 3072
#   define FINGERPRINT_RSA_KEYSIZE (ALG_RSA * (0x1ull << 45))
#elif MAX_RSA_KEY_BITS == 4096
#   define FINGERPRINT_RSA_KEYSIZE (ALG_RSA * (0x1ull << 46))
#else
#   error Unable to fingerprint RSA Keysize
#endif

// bits 48-55 = ECC key size fingerprint
#if (ECC_BN_P638 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 48))
#elif (ECC_NIST_P521 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 49))
#elif (ECC_NIST_P384 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 50))
#elif (ECC_NIST_P256 == YES || ECC_SM2_P256 == YES || ECC_BN_P256 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 51))
#elif (ECC_NIST_P224 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 52))
#elif (ECC_NIST_P192 == YES)
#   define FINGERPRINT_ECC_KEYSIZE (ALG_ECC * (0x1ull << 53))
#else
#   error Unable to fingerprint ECC Keysize
#endif

#define FINGERPRINT_GENERAL \
    ( FINGERPRINT_SPEC_VERSION | FINGERPRINT_ALGOS | FINGERPRINT_RSA_KEYSIZE | FINGERPRINT_ECC_KEYSIZE )

#define TPM_IMPLEMENTATION_FINGERPRINT \
    ( FINGERPRINT_ARCH | FINGERPRINT_GENERAL )
